...
# CPT-ovi (0 = negativno, 1 = pozitivno) 
# Konvencija za dimenzije: P_XY[x, y] = P(X=x | Y=y)
...
def draw_dag():
    ...
    ax.set_title("Bayesova mreža — DAG", fontsize=14, fontweight="bold")
    ax.axis("off")
    plt.tight_layout()
...
# Zadatak 1.2a — Variable Elimination
# Red eliminacije C → A → D (obrazloženje izbora u izveštaju)

def variable_elimination():
    # Faktori sa instantiranom evidencijom
    f_Ee_CD = P_ECD[0]    # P(e⁻|C,D),  shape [c, d]
    f_Ff_D  = P_FD[0]     # P(f⁻|D),    shape [d]

    # Korak 1 — eliminisati C
    # f1[a,d] = Σ_c P(C=c|A=a) · P(e⁻|C=c,D=d)
    f1 = np.einsum('ca,cd->ad', P_CA, f_Ee_CD)

    # Korak 2 — eliminisati A
    # f2[b,d] = Σ_a P(A=a)·P(B=b|A=a)·P(D=d|A=a,B=b)·f1[a,d]
    f2 = np.einsum('a,ba,dab,ad->bd', P_A, P_BA, P_DAB, f1)

    # Korak 3 — eliminisati D
    # f3[b] = Σ_d f2[b,d] · P(f⁻|D=d)
...
# Zadatak 1.2b — Rejection Sampling
# Topološko uzorkovanje (A → B,C → D → E,F), odbacivanje ako E≠e⁻ ili F≠f⁻
...
# Zadatak 1.2d — Gibbs Sampling
# Fiksiramo E=e⁻, F=f⁻ i ciklično uzorkujemo A,B,C,D iz uslovnih raspodela
# datih Markov blanketom (formula uz svaki korak ispod)

def gibbs_sampling(N, burn_in=200, rng=None):
    if rng is None:
        rng = np.random.default_rng()

    # Nasumična inicijalizacija skrivenih varijabli
...
# Zadatak 1.2 — Monte Carlo petlja (Nr=100)
# N_RS biramo analitički (std_RS = sqrt(p(1-p) / (P(e⁻,f⁻)·N))), a N_Gibbs
# empirijski (proceni std na N_cal=300 uzoraka pa skaliraj) — cilj: std ≈ 0.025

def run_monte_carlo(Nr=100, target_std=0.025, seed=42):
    ve_prob   = variable_elimination()
    ve_exact  = ve_prob[1]

    # P(e⁻, f⁻) — normalizacioni factor iz VE (= stopa prihvatanja za RS)
    ...
    # N za Gibbs (empirijski: proceni std na N_cal uzoraka, pa skaliraj)
    ...
    print(f"Ciljana std          ≈ {target_std:.3f}")
    print(f"P(e⁻, f⁻)           = {acc_rate:.4f}")
    print(f"N (Rejection)        = {N_rs}  (prihvaćeno ≈ {int(N_rs * acc_rate)})")
    ...
    for ax, est, title in configs:
        ...
        ax.axvline(ve_exact, color="crimson", lw=2,   ls="--",
                   label=f"VE (tačno) = {ve_exact:.4f}")
        ax.axvline(mean,     color="black",   lw=1.5, ls="-",
                   label=f"Mean = {mean:.4f},  Std = {std:.4f}")
        ax.set_title(title, fontsize=10)
        ax.set_ylabel("Broj realizacija")
        ax.legend(fontsize=9)

    axes[1].set_xlabel("Procena  P(b⁺ | e⁻, f⁻)")
    plt.suptitle(f"Monte Carlo procena  P(b⁺ | e⁻, f⁻),   Nᵣ = {Nr}",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    plt.savefig("src/homework_2/mc_histograms.png", dpi=150)
    plt.show()

    print(f"\n{'Metoda':<32} {'Mean':>8} {'Std':>8} {'N':>6}")
    print("─" * 56)
    ...
    print(f"{'VE (tačno)':<32} {ve_exact:>8.4f}")


# ZADATAK 2 — Čestični filter

# Zadatak 2.1 — Simulacija trajektorije
# v[t] ~ N(μ[t], (μ[t]/3)²), μ[t] = 2^(-t/10) — meta eksponencijalno usporava,
# a koeficijent varijacije brzine ostaje konstantan (= 1/3) tokom trajektorije
...
def simulate_trajectory(rng=None):
    ...
    t   = np.arange(T)
    mu  = 2.0 ** (-t / 10.0)          # srednja vrednost brzine, shape (T,)
    std = mu / 3.0                     # std brzine

    v = rng.normal(mu, std)            # v[t] ~ N(μ[t], (μ[t]/3)²)
    x = np.zeros(T + 1)
    for k in range(T):
        x[k + 1] = x[k] + v[k]

    return x[:T], v                    # pozicija x[0..49], brzina v[0..49]
...
def plot_trajectory(x, v):
    ...
    ax1.set_ylabel("Pozicija  x[t]")
    ax1.set_title("Stvarna trajektorija mete")
    ax1.grid(alpha=0.3)

    ax2.plot(t, v, color="#16A34A", lw=2)
    ax2.axhline(0, color="gray", lw=0.8, ls="--")
    ax2.set_ylabel("Brzina  v[t]")
    ax2.set_xlabel("Vreme  t")
    ax2.grid(alpha=0.3)

    # Prikazati μ[t] kao referentnu srednju vrednost
    mu = 2.0 ** (-t / 10.0)
    ax2.plot(t, mu,  color="crimson", lw=1.5, ls="--", label="μ[t] = 2^(-t/10)")
...
# Zadatak 2.2 — Generisanje opservacija
# e[t] = θ[t]·x[t] + n[t], θ ~ Bernoulli(0.9) (10% outlier-a kad senzor "ne vidi"
# metu), n ~ Laplace(0, b(x)); b(x) = sqrt(|x|)/5 — šum raste sa udaljenošću mete

def _meas_scale(x):
    """b(x[t]) = sqrt(|x[t]|) / 5  — skala Laplace šuma."""
    return np.sqrt(np.abs(x)) / 5.0


def generate_observations(x_true, rng=None):
    """Vraća (e, theta): opservacije i realizacije Bernoulli promenljive."""
    if rng is None:
        rng = np.random.default_rng()

    b     = _meas_scale(x_true)                  # shape (T,)
    theta = rng.binomial(1, 0.9, size=T)          # 1 = inlier, 0 = outlier
    n     = rng.laplace(0.0, np.maximum(b, 1e-9)) # Laplace šum
    e     = theta * x_true + n
    return e, theta


def observation_likelihood(e_t, x_particles):
    """Likelihood p(e[t] | x_i) za svaku česticu — vektorizovano."""
...
def plot_observations(x_true, e_obs, theta):
    ...
    ax.plot(t, x_true, color="#2563EB", lw=2, label="Stvarna pozicija x[t]")

    inliers  = theta == 1
    outliers = theta == 0
    ax.scatter(t[inliers],  e_obs[inliers],  s=20, color="#16A34A", alpha=0.75,
               label=f"Opservacija e[t]  (θ=1, n={inliers.sum()})")
    if outliers.any():
        ax.scatter(t[outliers], e_obs[outliers], s=60, color="crimson",
                   marker="x", linewidths=2,
                   label=f"Outlier  (θ=0, n={outliers.sum()})")

    ax.set_xlabel("Vreme  t")
    ax.set_ylabel("Pozicija")
    ax.set_title("Stvarna trajektorija i zašumljene opservacije  (Zadatak 2.2)")
    ax.legend(fontsize=9)
    ax.grid(alpha=0.3)
    plt.tight_layout()
...
# Zadatak 2.3 — Čestični filter
# Radimo u log-domenu (log_weights): 50 uzastopnih množenja malih
# verovatnoća bi u standardnoj aritmetici brzo dovelo do underflow-a

def particle_filter(e_obs, N=300, strategy='every', rng=None):
    """
    Vraca x_est (T,), particles_hist i weights_hist (za vizuelizaciju).
    ...
    log_weights = np.zeros(N)   # log(1) — uniformno, normalizujemo u svakom koraku

    x_est          = np.zeros(T)
    particles_hist = []          # za vizuelizaciju (Zadatak 2.4)
    weights_hist   = []

    for t in range(T):
        # 1. Azuriranje log-tezina opservacijom e[t]
        lik         = observation_likelihood(e_obs[t], particles)
        log_weights = log_weights + np.log(lik + 1e-300)

        # Normalizacija: log-sum-exp trik (oduzmi max, eksponenciraj, normalizuj)
        log_weights = log_weights - log_weights.max()
        weights     = np.exp(log_weights)
        weights     = weights / weights.sum()

        # 2. Procena pozicije: x_est[t] = sum_i w_i * x_i
        x_est[t] = float(np.dot(weights, particles))

        # 3. Sacuvaj za vizuelizaciju (pre reuzorkovanja)
        ...
        if do_resample:
            ...
            log_weights = np.zeros(N)   # reset: sve cestice jednake tezine

        # 5. Propagacija po modelu kretanja
        if t < T - 1:
...
def plot_pf_results(x_true, v_true, e_obs, x_est):
    ...
    # Estimacija brzine kao konacna razlika estimirane pozicije: v_est[t] ~ x_est[t]-x_est[t-1]
    v_est      = np.empty(T)
    ...
    # Gornji panel — pozicija
    ax1.scatter(t, e_obs, s=14, color="#94A3B8", alpha=0.65,
                label="Opservacije e[t]", zorder=1)
    ax1.plot(t, x_true, color="#2563EB", lw=2,
             label="Stvarna pozicija x[t]", zorder=2)
    ax1.plot(t, x_est,  color="crimson",  lw=2, ls="--",
             label="CF estimacija  x_est[t]", zorder=3)
    ax1.set_ylabel("Pozicija")
    ax1.legend(fontsize=9)
    ax1.grid(alpha=0.3)
    ax1.set_title("Cestični filter — pozicija i brzina  (Zadatak 2.3)")

    # Donji panel — brzina
    ax2.plot(t, v_true, color="#2563EB", lw=2, label="Stvarna brzina v[t]")
    ax2.plot(t, v_est,  color="crimson",  lw=2, ls="--",
             label="Estimirana brzina (delta x_est)")
    ax2.set_ylabel("Brzina")
    ax2.set_xlabel("Vreme  t")
    ax2.legend(fontsize=9)
    ax2.grid(alpha=0.3)
...
# Zadatak 2.4 — Vizuelizacija čestica
# Prikazujemo stanje PRE reuzorkovanja — tada težine nose informaciju
# (posle reuzorkovanja su sve ≈ 1/N, pa je raspodela manje informativna)
...
def plot_particles(x_true, e_obs, particles_hist, weights_hist,
    ...
    # constrained_layout=True: matplotlib automatski izracunava razmake
    # (uklj. suptitle i colorbars) — nema rucnog subplots_adjust
    fig, axes = plt.subplots(n, 1, figsize=(12, 3.5 * n),
                              constrained_layout=True)

    fig.suptitle("Vizuelizacija čestica pre reuzorkovanja  (Zadatak 2.4)",
                 fontsize=13, fontweight="bold")
    ...
    for idx, (ax, t) in enumerate(zip(axes, time_steps)):
        ...
        ax.set_yticks([])
        ax.set_xlabel("Pozicija" if idx == n - 1 else "", fontsize=10)
...
# Zadatak 2.5 — Monte Carlo analiza RMSE
# Fer poređenje: ista trajektorija i opservacije za sve 3 strategije u svakoj
# MC iteraciji, tako da razlike u RMSE potiču samo od reuzorkovanja

def run_mc_particle_filter(Nr=500, N=100, seed=7):
    rng_master = np.random.default_rng(seed)
    # Pre-generisati sve seedove: [traj+obs, none, every, adaptive] po iteraciji
    ...
    for r in range(Nr):
        ...
        if (r + 1) % 100 == 0:
            print(f"  {r + 1}/{Nr} simulacija gotovo...")

    return rmse   # shape (3, Nr)


def plot_mc_rmse(rmse):
    labels = ['Bez reuzorkovanja\n(none)',
              'Svaki korak\n(every)',
              'Adaptivno\n(Neff < N/2)']
    ...
    ax.set_ylabel("RMSE pozicije")
    ax.set_title("MC analiza RMSE za 3 strategije reuzorkovanja  (Nr=500, N=100)",
                 fontsize=11)
    ...
    print(f"\n{'Strategija':<22} {'Mean':>8} {'Std':>8} {'Median':>8} {'Max':>8}")
    print("─" * 54)
    for name, d in zip(['Bez reuzorkovanja', 'Svaki korak', 'Adaptivno'], data):
...
if __name__ == "__main__":
    ...
    print("\n─── Zadatak 2.2 — Opservacije ───────────────")
    # Odvojen seed za opservacije — menjaj OBS_SEED da testiras razlicite realizacije
    OBS_SEED = 0
    rng_obs = np.random.default_rng(OBS_SEED)
    e_obs, theta = generate_observations(x_true, rng_obs)
    n_outliers = int((theta == 0).sum())
    print(f"Outlieri (theta=0): {n_outliers} / {T}  (ocekivano ~{int(T * 0.1)}, seed={OBS_SEED})")
    print(f"e[0]  = {e_obs[0]:.3f},  e[49] = {e_obs[49]:.3f}")
    plot_observations(x_true, e_obs, theta)

    print("\n─── Zadatak 2.3 — Cestični filter ──────────")
    ...
    print(f"RMSE pozicije (N=300, resample svaki korak): {rmse_pf:.4f}")
    plot_pf_results(x_true, v_true, e_obs, x_est_pf)

    print("\n─── Zadatak 2.4 — Vizuelizacija cestica ────")
    plot_particles(x_true, e_obs, particles_hist, weights_hist,
                   time_steps=(2, 11, 22, 35, 49))

    print("\n─── Zadatak 2.5 — MC analiza RMSE (Nr=500) ─")
